...
export default async function ListingPage(props: { params: Promise<{ slug: string }> }) {
  ...
  const display = await resolveListingDisplay(listing.categoryId, listing.attributes)
            ...
            <ul className="flex flex-wrap gap-2">
              {display.highlights.map((attribute) => (
                ...
                >
                  ...
                  <span className="font-medium">{attribute.display}</span>
                </li>
              ))}
            </ul>
          ) : null}
