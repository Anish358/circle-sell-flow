...
async function seed() {
  await db.transaction(async (tx) => {
    ...
    // Photos, in one insert. Ids come back from the statement above rather than from a
    // lookup per listing — the listing id is a uuid generated by the database, so there
    // is nothing to know in advance.
    const listingIds = new Map(inserted.map((row) => [row.slug, row.id]))
    const images = LISTINGS.flatMap((listing) =>
      (listing.images ?? []).map((image, index) => ({
        listingId: idFor(listingIds, listing.slug, "listing"),
        url: image.url,
        alt: image.alt,
        // Sorted from zero: the first image is the primary one, which is an ordering
        // rule rather than an `is_primary` flag two rows could both claim.
        sort: index,
      })),
    )

    if (images.length > 0) await tx.insert(schema.listingImages).values(images)
  })
